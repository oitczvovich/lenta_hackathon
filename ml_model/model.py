...
CURRENT_DIR = os.path.join(os.getcwd(), 'ml_model')
...
# # 0. ЗАГРУЖАЕМ МОДЕЛИ   # импорт перенесен в функцию get_prediction
...
# ПОДГОТОВКА ТЕСТОВОЙ ВЫБОРКИ РАЗБИТОЙ НА ГРУППЫ:
    ...
    def get_prediction(features_test_gr_1,
        ...
        # перенес сюда для запуска forecast
